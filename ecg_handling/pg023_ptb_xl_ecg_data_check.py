...
ptb_xl_df['tfrecord_path'] = "/mnt/s/Workfolder/vectorembedding/tfrecord/ptb_xl/id_" + ptb_xl_df["ecg_id"].astype(str).str.zfill(10) 

tf_dataset = tf.data.TFRecordDataset(ptb_xl_df['tfrecord_path']).map(parse).repeat(1).batch(256).prefetch(tf.data.experimental.AUTOTUNE)
...
ptb_xl_frag_df = pd.concat([ptb_xl_df,tfrecord_check_flags_df],axis=1)
joblib.dump(ptb_xl_frag_df,"/mnt/s/Workfolder/vectorembedding/df/ptb_xl_frag_df")
