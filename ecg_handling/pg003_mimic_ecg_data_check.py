...
def parse(example):
    ...
    ecg = tf.reshape(ecg_, tf.stack([5000, 12, 1]))    
    return ecg


# Load pandas DataFrame
mimic_iv_ecg_data_df = joblib.load("/mnt/s/Workfolder/vectorembedding/df/mimic_iv_ecg_data_df")
mimic_iv_ecg_data_df['tfrecord_path'] = "/mnt/s/Workfolder/vectorembedding/tfrecord/mimic_iv_ecg/" + mimic_iv_ecg_data_df["subject_id"] + '_' + mimic_iv_ecg_data_df["ecg_id"]

tf_dataset = tf.data.TFRecordDataset(mimic_iv_ecg_data_df['tfrecord_path']).map(parse).repeat(1).batch(256).prefetch(tf.data.experimental.AUTOTUNE)
...
mimic_iv_ecg_data_frag_df = pd.concat([mimic_iv_ecg_data_df,tfrecord_check_flags_df],axis=1)
joblib.dump(mimic_iv_ecg_data_frag_df,"/mnt/s/Workfolder/vectorembedding/df/mimic_iv_ecg_data_frag_df")
